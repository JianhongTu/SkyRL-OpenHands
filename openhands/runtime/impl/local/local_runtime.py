...
def check_dependencies(code_repo_path: str, env_root_path: str) -> None:
    ERROR_MESSAGE = 'Please install OpenHands dependencies into the active environment, e.g. `uv sync --group runtime`.'
    ...
    if not os.path.exists(env_root_path):
        raise ValueError(
            f'Environment path {env_root_path} does not exist. ' + ERROR_MESSAGE
        )
