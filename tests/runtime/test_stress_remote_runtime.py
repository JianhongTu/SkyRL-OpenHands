...
.venv/bin/pytest -vvxss tests/runtime/test_stress_remote_runtime.py
```
...
def test_stress_remote_runtime_long_output_with_soft_and_hard_timeout():
    ...
    try:
        ...
        for i in range(10):
            ...
            mem_action = CmdRunAction(
                'ps aux | awk \'{printf "%8.1f MB  %s\\n", $6/1024, $0}\' | sort -nr | grep "action_execution_server" | grep "/openhands/micromamba" | grep -v grep | awk \'{print $1}\''
            )
            mem_obs = runtime.run_action(mem_action)
            assert mem_obs.exit_code == 0
            logger.info(
